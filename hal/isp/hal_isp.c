...
//								debg("ddc_class:%d\n",isp.p_ddc->ddc_class);
        ...
        case _DGAIN_POS_ 	 :	debg("write dgain\n");
								//	memcpy(isp.p);
								memcpy(isp.p_rgb_dgain,pbdata+startPos,trans_len);
        ...
        case _RGB_GAMA_POS_  : 	debg("write rgb_gamma\n");
								memcpy(isp.p_rgb_gama,pbdata+startPos,trans_len);
...
void hal_Sensor_common_Set(u8 *pbdata ,u32 trans_len, u32 param_3B){
	...
	switch(startPos){
		...
		case offsetof(COMMON,rotate):		CopyToMember(&common_adapt,offsetof(COMMON,rotate),
											...
											_SET_BIT_(commom_mode,common_mode_max,1);
													break;											
		case offsetof(COMMON,online):		CopyToMember(&common_adapt,offsetof(COMMON,online),
													sizeof(common_adapt.online),pbdata,startPos,trans_len);
...
void ygama_update(u8 *pbdata ,u32 trans_len, u32 param_3B)
{
	...
	CopyToMember(isp.p_ygama, offsetof(YGAMA,gma_num), 
			sizeof(isp.p_ygama->gma_num), pbdata, startPos, trans_len);		
